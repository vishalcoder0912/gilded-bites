...
const Cart = () => {
  ...
  const delivery = subtotal > 250000 ? 0 : 15000;
  const discount = 0;
  const total = subtotal + delivery - discount;

  useEffect(() => {
    if (isAuthenticated && !authLoading) fetchCart();
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
              ...
              <div className="space-y-4">
                {items.map((item) => (
                  <div key={item.id} className="grid gap-4 border-b border-[#d9a35b]/12 pb-4 last:border-b-0 last:pb-0 sm:grid-cols-[96px_1fr_auto] sm:items-center">
                    <ProductImage product={item.product} alt={item.product.name} className="h-28 w-full rounded-sm object-cover sm:h-24 sm:w-24" loading="lazy" />
                    <div className="min-w-0">
                      <h2 className="font-serif text-2xl text-[#f8eadc]">{item.product.name}</h2>
                      <p className="mt-1 text-sm text-[#f0c27a]">{formatINRFromPaise(item.priceSnapshot)}</p>
                      <div className="mt-4 flex items-center gap-3">
                        <div className="flex items-center rounded-sm border border-[#d9a35b]/22">
                          <button type="button" onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-2 text-[#c8b5a4] hover:text-[#f0c27a]" aria-label="Decrease quantity">
                            <Minus className="h-3.5 w-3.5" />
                          </button>
                          <span className="w-9 text-center text-sm">{item.quantity}</span>
                          <button type="button" onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-2 text-[#c8b5a4] hover:text-[#f0c27a]" aria-label="Increase quantity">
                            <Plus className="h-3.5 w-3.5" />
                          </button>
                        </div>
                        <button type="button" onClick={() => removeFromCart(item.id)} className="text-[#c8b5a4] transition hover:text-[#d94f35]" aria-label="Remove item">
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </div>
                    </div>
                    <div className="font-serif text-2xl text-[#f0c27a] sm:text-right">{formatINRFromPaise(item.priceSnapshot * item.quantity)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <aside className="h-fit rounded-sm border border-[#d9a35b]/22 bg-[#140904]/82 p-6 lg:sticky lg:top-28">
              ...
              <div className="mt-6 space-y-3 text-sm">
                <div className="flex justify-between text-[#c8b5a4]"><span>Subtotal</span><span>{formatINRFromPaise(subtotal)}</span></div>
                <div className="flex justify-between text-[#c8b5a4]"><span>Delivery</span><span>{delivery ? formatINRFromPaise(delivery) : "Complimentary"}</span></div>
                <div className="flex justify-between text-[#6fbf73]"><span>Discount</span><span>-{formatINRFromPaise(discount)}</span></div>
                <div className="mt-4 border-t border-[#d9a35b]/14 pt-4">
                  <div className="flex justify-between font-serif text-2xl text-[#f8eadc]"><span>Total</span><span className="text-[#f0c27a]">{formatINRFromPaise(total)}</span></div>
                  <p className="mt-2 text-xs leading-relaxed text-[#c8b5a4]">Tax included and shipping calculated at checkout.</p>
                </div>
              </div>
              <button type="button" onClick={() => navigate("/checkout")} className="btn-gold mt-7 w-full">Proceed to Checkout</button>
